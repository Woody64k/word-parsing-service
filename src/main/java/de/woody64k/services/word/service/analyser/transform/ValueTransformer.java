...
import de.woody64k.services.word.model.value.request.transform.SubStringTransform;
            ...
            if (requ.getStartWords() != null) {
                for (String startWord : requ.getStartWords()) {
                    if (value.contains(startWord)) {
                        String textAfterStart = value.substring(value.indexOf(startWord) + startWord.length()).trim();
        ...
        if (requ.getEndWords() != null) {
            for (String endWord : requ.getEndWords()) {
                if (value.contains(endWord)) {
                    return value.substring(0, value.indexOf(endWord)).trim();
                }
            }
        }
        return value;
    ...
    public static Object doSplit(String value, ValueTransformRequirement requ) {
        if (value != null && requ.getSplit() != null) {
            ...
            if (requ.getSplit().getAs() == SplitAs.list) {
                ...
                for (SplitValueRequirement splitValRequ : requ.getSplit().getValues()) {
